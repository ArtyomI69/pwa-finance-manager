import { PurchaseItem } from '@/shared/types/shopGroup';

export function getCategoryStats(items: PurchaseItem[]): {
  name: string;
  amount: number;
  share: string;
  color: string;
}[] {
  // Группируем товары по категориям и суммируем суммы
  const categoryMap = new Map<number, { name: string; amount: number }>();

  items.forEach((item) => {
    const categoryId = item.category_id;
    const currentAmount = categoryMap.get(categoryId)?.amount || 0;
    categoryMap.set(categoryId, {
      name: item.categories.name,
      amount: currentAmount + item.sum,
    });
  });

  // Преобразуем Map в массив
  const categoryStats = Array.from(categoryMap.values());

  // Вычисляем общую сумму для расчета долей
  const totalAmount = categoryStats.reduce((sum, item) => sum + item.amount, 0);

  // Массив цветов для категорий (можно расширить по необходимости)
  const colors = [
    'bg-cyan-500 dark:bg-cyan-500', // cyan
    'bg-blue-500 dark:bg-blue-500', // blue
    'bg-emerald-500 dark:bg-emerald-500', // emerald
    'bg-violet-500 dark:bg-violet-500', // violet
    'bg-fuchsia-500 dark:bg-fuchsia-500', // fuchsia
    'bg-amber-500 dark:bg-amber-500', // amber
    'bg-pink-500 dark:bg-pink-500', // pink (замена rose)
    'bg-lime-500 dark:bg-lime-500', // lime (новый)
  ];

  // Сортируем по убыванию суммы
  const sortedStats = categoryStats.sort((a, b) => b.amount - a.amount);

  // Формируем результат с расчетом долей и подбором цвета
  const result = sortedStats.map((stat, index) => {
    const share = ((stat.amount / totalAmount) * 100).toFixed(1) + '%';
    // Используем цвет по индексу, если цветов меньше чем категорий - используем modulo
    const color = colors[index % colors.length];

    return {
      name: stat.name,
      amount: stat.amount,
      share: share,
      color: color,
    };
  });

  return result;
}
